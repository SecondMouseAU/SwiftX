...
struct TextParser {
    ...
    private mutating func next() -> String? {
        while i < b.count {
            ...
            var w = [UInt8]()
            while i < b.count && !isWS(b[i]) && !isPunct(b[i]) {
                w.append(b[i])
                i += 1
            }
    ...
    private mutating func parseMesh(_ meshes: inout [X.RawMesh]) {
        ...
        for _ in 0..<nf {
            ...
            for _ in 0..<k {
                guard let v = readInt() else { return }
                idx.append(v)
            }
            // fan; skip lines/points
            if k >= 3 { for j in 1..<(k - 1) { m.tris.append((idx[0], idx[j], idx[j + 1])) } }
        }
        var depth = 1  // skip remainder of the Mesh block
        while depth > 0, let tk = next() {
            if tk == "{" { depth += 1 } else if tk == "}" { depth -= 1 }
        }
        meshes.append(m)
    }
...
/// Reads the binary `.x` token stream and extracts every `Mesh` object's geometry.
///
/// Integers and floats arrive batched in INTEGER_LIST / FLOAT_LIST tokens; `readInt`/`readFloat`
/// drain those buffers, refilling from the next list token, so the same count-driven Mesh logic as
/// the text path applies.
struct BinaryParser {
    let b: [UInt8]
    var p: Int
    let floatBytes: Int

    init(bytes: [UInt8], start: Int, floatBytes: Int) {
        b = bytes
        p = start
    ...
    private enum Tok {
        ...
        case obrace, cbrace, template, end, other
    }

    private mutating func u16() -> Int {
        let v = Int(b[p]) | (Int(b[p + 1]) << 8)
        p += 2
        return v
    }
    ...
    private mutating func f32() -> Float {
        let bits =
            UInt32(b[p]) | (UInt32(b[p + 1]) << 8) | (UInt32(b[p + 2]) << 16)
            | (UInt32(b[p + 3]) << 24)
        p += 4
    ...
    private mutating func token() -> Tok {
        ...
        case 1:
            let n = u32()
            let s = String(decoding: b[p..<min(p + n, b.count)], as: UTF8.self)
            p += n
            return .name(s)
        case 2:
            let n = u32()
            p += n
            if p + 2 <= b.count { _ = u16() }
            return .other  // STRING + terminator
        ...
        case 6:
            ...
            var a = [Int]()
            a.reserveCapacity(n)
            for _ in 0..<n { a.append(u32()) }
            return .intList(a)
        case 7:
            let n = u32()
            var a = [Float]()
            a.reserveCapacity(n)
            for _ in 0..<n { a.append(floatBytes == 8 ? f64() : f32()) }
            return .floatList(a)
    ...
    private var intBuf = [Int]()
    private var ii = 0
    private var fBuf = [Float]()
    private var fi = 0
    private mutating func resetBuffers() {
        intBuf = []
        ii = 0
        fBuf = []
        fi = 0
    ...
    private mutating func readInt() -> Int? {
        while ii >= intBuf.count {
            ...
            case .intList(let a) where !a.isEmpty:
                intBuf = a
                ii = 0
            case .end: return nil
            default: continue
    ...
    private mutating func readFloat() -> Float? {
        while fi >= fBuf.count {
            switch token() {
            case .floatList(let a) where !a.isEmpty:
                fBuf = a
                fi = 0
            case .end: return nil
            default: continue
    ...
    mutating func parse() -> [X.RawMesh] {
        ...
        loop: while true {
            ...
            // else: descend to find nested Meshes
            case .name(let s): if s == "Mesh" { parseMesh(&meshes) }
            default: break
            }
        }
        return meshes
