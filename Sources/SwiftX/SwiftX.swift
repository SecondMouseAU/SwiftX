...
/// binary). SwiftX reads **all four**, plus 32- and 64-bit float variants. It extracts **geometry
/// only** (vertex positions and triangle faces, with polygon faces fan-triangulated) across every
/// `Mesh` object in the file, and skips templates, frames' non-geometry data, normals, materials and
/// textures.
///
/// ```swift
...
public enum X {
    ...
    public struct Options: Sendable {
        ...
        ///
        /// `.x` files duplicate positions at material seams and between the file's multiple
        /// `Mesh` blocks; welding restores connectivity. `nil` keeps every authored vertex.
        /// Default `1e-4`.
        ...
        ///
        /// `.x` carries no unit; RailSim authors in metres. Default `1`.
        public var scale: Float
        /// Negate Z and reverse winding.
        ///
        /// DirectX is nominally left-handed, but `.x` files are authored in many conventions;
        /// leave `false` (faithful) unless your target needs the flip. Default `false`.
        public var flipZ: Bool

        public init(
            weldEpsilon: Float? = 1e-4, dropDegenerate: Bool = true, scale: Float = 1,
            flipZ: Bool = false
        ) {
    ...
    public static func read(data: Data, options: Options = .default) throws -> Mesh {
        ...
        guard bytes[0] == 0x78, bytes[1] == 0x6F, bytes[2] == 0x66, bytes[3] == 0x20 else {
        ...
        func parseText(_ src: [UInt8], _ s: Int) -> [RawMesh] {
            var p = TextParser(bytes: src, start: s)
            return p.parse()
        }
        func parseBin(_ src: [UInt8], _ s: Int) -> [RawMesh] {
            var p = BinaryParser(bytes: src, start: s, floatBytes: floatBytes)
            return p.parse()
        }
    ...
    static func assemble(_ meshes: [RawMesh], options: Options) -> Mesh {
        ...
        for m in meshes {
            for (a, b, c) in m.tris {
                guard a < m.positions.count, b < m.positions.count, c < m.positions.count else {
                    continue
                }
                let tri =
                    options.flipZ
                    ? [m.positions[a], m.positions[c], m.positions[b]]
    ...
    static func mszipDecompress(_ b: [UInt8], start: Int) throws -> [UInt8] {
        ...
        var out = [UInt8]()
        while p + 4 <= b.count {
            let size = Int(b[p]) | (Int(b[p + 1]) << 8)
            p += 2
            let magic = Int(b[p]) | (Int(b[p + 1]) << 8)
            p += 2
    ...
    /// Raw DEFLATE (RFC 1951) via Apple's `Compression` framework.
    ///
    /// `COMPRESSION_ZLIB` is headerless raw DEFLATE. Unavailable on non-Apple platforms.
            ...
            let n = dst.withUnsafeMutableBufferPointer { d in
                src.withUnsafeBufferPointer { s in
                    compression_decode_buffer(
                        d.baseAddress!, cap, s.baseAddress!, src.count, nil, COMPRESSION_ZLIB)
                }
            }
